...
double _window_begin(std::string_view content) {
    ...
    const ImVec2 defaultSize(620.0f * uiScale, 460.0f * uiScale);
    if (cfg.fitWindowToContent) {
        const ImGuiViewport* viewport = ImGui::GetMainViewport();
        const float width = ImMin(620.0f * uiScale, viewport->WorkSize.x * 0.92f);
        const float height = ImMin(cfg.fittedWindowHeight * uiScale,
    ...
    style.WindowPadding = { 8.0f * uiScale, 8.0f * uiScale };
    style.WindowBorderSize = 0.0f;
    style.FrameRounding = 0.0f;
    style.ItemInnerSpacing = { 4.0f * uiScale, 4.0f * uiScale };
    style.FrameBorderSize = 1.5f * uiScale;
    style.FramePadding = { 4.0f * uiScale, 2.0f * uiScale };
    style.ItemSpacing = { 4.0f * uiScale, 2.0f * uiScale };
