...
void startDashing(PlayerObject* player, DashRingObject* ring) {
    ...
    cocos2d::CCPoint p = CCPoint{
        static_cast<float>(std::cos(dashAngle * 0.01745329f)),
        static_cast<float>(std::sin(dashAngle * 0.01745329f)),
    };
#endif
...
void ringJump(PlayerObject* player, RingObject* ring) {
    ...
    // Trajectory simulation must not emit gameplay events.
    ...
    if (ring->m_objectType == GameObjectType::CustomRing) {
        // ponytail: custom-ring effects are omitted until trajectory needs them.
        return;
    }
